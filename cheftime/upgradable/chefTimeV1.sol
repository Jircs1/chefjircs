...
contract ChefTime is Initializable, ERC721EnumerableUpgradeable, OwnableUpgradeable, ReentrancyGuard {
    ...
    function initialize(
        ...
        __ERC721_init("ChefTime", "CT");
        __Ownable_init();
        contractVersion = 1; // Initialize contract version here
    ...
    function buyChefTime(uint256 _tokenId, string memory _url)
        external
        payable
        nonReentrant
    {
    ...
    function computeExtendChefTime(uint8 _numDays, uint256 _tokenId)
        public
        view
        returns (uint256)
    {
    ...
    function extendChefTime(uint8 _numDays, uint256 _tokenId)
        external
        payable
        nonReentrant
    {
        ...
        TokenInfo storage token = tokenInfo[_tokenId];
        uint256 requiredEth = computeExtendChefTime(_numDays, _tokenId);
    ...
    function refundChefTime(uint256 _tokenId) external nonReentrant {
        require(_exists(_tokenId), "Token does not exist");
        TokenInfo storage token = tokenInfo[_tokenId];
